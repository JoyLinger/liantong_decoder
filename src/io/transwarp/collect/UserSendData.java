  ...
  */
  private void run(String sourcedir) {
    ...
    try {
      ...
      sendMessages(lines);
      sendErrorMessages("file@@" + sourcedir + "@@" + sdf.format(new Date()) + "@@" + RightFlag);
      moveFile(sourcedir, ".complete");
    } catch (Exception e) {
      e.printStackTrace();
      sendErrorMessages("file@@" + sourcedir + "@@" + sdf.format(new Date()) + "@@" + ErrorFlag);
  ...
  private void sendErrorMessages(String messages) {
    ...
    producer.send(new KeyedMessage<String, String>(topic_id, UUID.randomUUID().toString(), messages));
  }
...
//               处理压缩包
  ...
  public static void main(String[] args) {
    ...
    Long start = 0L;
    Long end = 0L;
    UserSendData sd = new UserSendData();
