...
public class FormatIuPSData {
  ...
  static {
    ...
    InputStream pis = FormatIuPSData.class.getClassLoader().getResourceAsStream("param.properties");
    try {
      paramProp.load(pis);
    } catch (IOException e) {
  ...
  private FormatIuPSData() {
    InputStream is = FormatIuPSData.class.getClassLoader().getResourceAsStream("topic.properties");
...
//            p_check.load(is_check);
  ...
  public static void main(String[] args) {
    ...
    FormatIuPSData fxd = new FormatIuPSData();
    for (String filePath : args) {
      if (!new File(filePath.substring(0, filePath.lastIndexOf("/"))).exists()) {
        LOGGER.warn("File parent directory doesn't exist.");
        continue;
      }
      if (!filePath.contains("IuPS")) {
        LOGGER.warn("File '" + filePath + "' is not IuPS");
        continue;
      }
      es.execute(new FormatIuPSData.MyThread(filePath, fxd));
    }
    es.shutdown();
  }
  ...
  private static class MyThread extends Thread {
    ...
    private MyThread(String filePath, FormatIuPSData fxd) {
      this.fp = filePath;
      this.fxd = fxd;
    }
